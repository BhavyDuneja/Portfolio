...
import { Calendar, Clock, User, Tag, ArrowLeft, Video, Eye } from 'lucide-react'
import { getPostById, getRelatedPosts } from '@/lib/storage'
import ShareAndView from './ShareAndView'
...
export default async function BlogPostPage({ params }: { params: { id: string } }) {
  ...
  const relatedPosts = await getRelatedPosts(params.id, 3)

  const imageUrl = post.imageUrl || post.fullImageUrl || post.thumbnailUrl || null
          ...
          <div className="p-8 md:p-12">
            ...
            <div className="flex items-center gap-3 mb-6">
              <Link
                ...
                className="px-3 py-1 rounded-full text-sm font-medium bg-saffron-500/10 text-saffron-400 border border-saffron-500/20 hover:bg-saffron-500/20 transition-colors"
              >
                {post.category}
              </Link>
              {post.featured && (
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-6 leading-tight font-display">
              {post.title}
            </h1>
            ...
            {/* Excerpt */}
            <div className="mb-8">
              <p className="text-lg text-gray-300 leading-relaxed italic border-l-2 border-saffron-500 pl-6">
                {post.excerpt}
              </p>
            </div>
            ...
            {/* Tags — each tag links to blog list filtered by tag */}
                ...
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
                    <Link
                      ...
                      className="px-3 py-1.5 bg-white/5 text-gray-400 text-sm rounded-full hover:bg-saffron-500/10 hover:text-saffron-400 transition-colors"
                    >
                      #{tag}
                    </Link>
                  ))}
            ...
            {/* Share + view tracking (client island) */}
            <ShareAndView slug={post.slug} title={post.title} />
          </div>
        </article>

        {/* Related Posts — server rendered, internal linking */}
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {relatedPosts.map((related) => (
                <Link key={related.id} href={`/blog/${related.slug}`}>
                  <article className="glass-card rounded-2xl overflow-hidden group cursor-pointer h-full hover:border-saffron-500/20 hover:shadow-lg hover:shadow-saffron-500/5 transition-all duration-500">
                    ...
                    <div className="p-5">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-saffron-500/10 text-saffron-400 border border-saffron-500/20">
                          {related.category}
                        </span>
                      </div>
                      <h3 className="text-white font-semibold mb-2 line-clamp-2 group-hover:text-saffron-400 transition-colors duration-300 text-sm">
                        {related.title}
                      </h3>
